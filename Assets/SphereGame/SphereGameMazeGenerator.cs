...
public class SphereGameMazeGenerator : MonoBehaviour
{
    ...
    public SphereGameMazeGrid grid; // get; private set; }
    ...
    private IEnumerator PrimGenerator()
    {
        ...
        List<SphereGameMazeGrid.Cell> frontier = grid.GetFloorNeighbors(start, includeUnvisitedOnly: true);
        ...
        while (allFloors.Count > 0)
        {
            ...
            {
                SphereGameMazeGrid.Cell neighbor = visitedNeighbors[Random.Range(0, visitedNeighbors.Count)];
    ...
    private IEnumerator RecursiveBacktrackGenerator()
    {
        ...
        Stack<SphereGameMazeGrid.Cell> stack = new Stack<SphereGameMazeGrid.Cell>();
        stack.Push(start);

        while (stack.Count > 0)
        {
            SphereGameMazeGrid.Cell current = stack.Peek();
            var unvisitedNeighbors = grid.GetFloorNeighbors(current, includeUnvisitedOnly: true);

            if (unvisitedNeighbors.Count > 0)
            {
                SphereGameMazeGrid.Cell next = unvisitedNeighbors[Random.Range(0, unvisitedNeighbors.Count)];
