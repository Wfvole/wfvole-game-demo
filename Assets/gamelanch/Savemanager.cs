...
public class SaveManager : UnitySingleton<SaveManager>
{
    ...
    public class PlayData
    {
        ...
        public string[] readyPlayerModels;
        //保存设置
        public bool enableCHPM;
        public int maxloopCHPM;
        public float volume;
        public int mazeGSpeed;
        public float rendererQuality;
    }
    ...
    public override void Awake()
    {
        ...
        {
            // 设置默认值
            playData.playerModel = "妖精爱莉";
            playData.enableCHPM = true;
            playData.gameMode = ThirdPersonShooterController.GameMode.Paradise;
            playData.maxloopCHPM = 2;
            playData.volume = 1f;
            playData.mazeGSpeed = 10;
            playData.rendererQuality = 0.8f;
            playData.readyPlayerModels = new string[] { "妖精爱莉" };
            // 其他默认值...
    ...
    {
        playData.enableCHPM=GameplayManager.Instance.enableGHPM;
        playData.maxloopCHPM=GameplayManager.Instance.loopGHPM;
        GameObject a = GameObject.FindWithTag("audioSource");
        if (a != null)
        {
            if (a.TryGetComponent<AudioSource>(out var v)) playData.volume = v.volume;
        };
        playData.mazeGSpeed=MazeGame.Instance.generator.generateSpeed;
        playData.rendererQuality=QualityController.Instance.targetScale;
        SaveToFile(SAVE_FILE, playData);
    }
    public void ReadSettings()
    {
        ...
        GameplayManager.Instance.enableGHPM =playData.enableCHPM;
        GameplayManager.Instance.loopGHPM = playData.maxloopCHPM;
        GameObject a = GameObject.FindWithTag("audioSource");
        if (a != null)
        {
            if (a.TryGetComponent<AudioSource>(out var v)) v.volume = playData.volume;
        };
        if (playData.mazeGSpeed <= 0) playData.mazeGSpeed = 10;
        MazeGame.Instance.generator.generateSpeed=playData.mazeGSpeed;
        QualityController.Instance.targetScale=playData.rendererQuality;
    }
    ...
    public void LoadPlayDate(string s)//仅作特殊引用
    {
        ...
        GameplayManager.Instance.readyPlayerModels = playData.readyPlayerModels.ToList() ;
    }
    IEnumerator LoadPos()
    {
        yield return null;//等载入完成
        yield return new WaitForEndOfFrame();
        ThirdPersonShooterController.Instance.playerTransform.SetPositionAndRotation(playData.playerPos, playData.playerRot);
    ...
    public void SavePlayDate()
    {
        ...
        playData.readyPlayerModels=GameplayManager.Instance.readyPlayerModels.ToArray();
        if(playData.iswear)
    ...
    public static mazegrid ExtractMazeGridData(MazeGrid source)
    {
        ...
        mazegrid result = new()
        {
            width = source.width,     // 保存房间尺寸，也可不保存
            length = source.length,
            mzgd = new int[rows, cols]
        };
