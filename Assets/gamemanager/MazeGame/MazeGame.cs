...
public class MazeGame : UnitySingleton<MazeGame>
{
    ...
    void Start()
    {
        ...
        //发送初始化完成讯号
        Eventmanager.Instance.Emit("MGInited",true);
    }
    ...
    void Setup()
    {
        ...
        airwall.tag = "mazegb";
        if (airwall.TryGetComponent<MeshRenderer>(out var amwr)) amwr.enabled = false;
        // 实例化玩家
        if (playerPrefab != null)
        {
            Instantiate(playerPrefab, startCell.position + Vector3.up * 2f, Quaternion.identity);
        }
        // 实例化出口标记
        if (exitMarker != null)
        {
        ...
        StartCoroutine(DelayEnbale());
    }
    IEnumerator DelayEnbale()
    {
        yield return null;
        GlobalHairPhysics.Instance.enabled = GameplayManager.Instance.enableGHPM;
        GlobalHairPhysics.Instance.constraintIterations = GameplayManager.Instance.loopGHPM;
    ...
    private void SpawnSpeedBoosts(int count)
    {
        ...
        for (int i = 0; i < floorCells.Count; i++)
        {
            ...
            (floorCells[rand], floorCells[i]) = (floorCells[i], floorCells[rand]);
        }
        // 生成道具
        for (int i = 0; i < Mathf.Min(count, floorCells.Count); i++)
        {
            ...
            sgo.tag = "mazegb";
            if(!sgo.TryGetComponent<SpeedBoostItem>(out _)) sgo.AddComponent<SpeedBoostItem>();
        }
    }

    private List<MazeGrid.Cell> GetAllFloorCellsExcludingStartAndExit(MazeGrid grid)
    {
        List<MazeGrid.Cell> lmgc= new();
        MazeGrid gd=grid;
    ...
    public List<Vector3> GetRandomPatrolPath(int pathLength, int maxBacktracks = 3)
    {
        ...
        List<MazeGrid.Cell> resultR = new();
        MazeGrid grid = generator.grid;
