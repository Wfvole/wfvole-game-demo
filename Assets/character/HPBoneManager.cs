...
public class HPBoneManager : MonoBehaviour
{
    ...
    public Vector3 gravity = new(0, -9.8f, 0); // 重力向量
    ...
    // 内部数据
    private List<HairPhysicalBone> hpbones = new();
    ...
    public void SetupHPBones()
    {
        ...
        for (int i = 0; i < hairBones.Count; i++)
        {
            ...
            if (!t.TryGetComponent<HairPhysicalBone>(out _))
                t.gameObject.AddComponent<HairPhysicalBone>();
        }
        ...
        {
            HairPhysicalBone b = hpbones[i];
            if (!b.TryGetComponent<HPBCollider>(out var capsule)) capsule = b.gameObject.AddComponent<HPBCollider>();
    ...
    {
        if (!t.TryGetComponent<HairPhysicalBone>(out var p)) return; // 实际不应发生
    ...
    private void InitTransforms()
    {
        ...
        {
            HairPhysicalBone p = hpbones[i];
            p.bone.SetLocalPositionAndRotation(p.initialLocalPos, p.initialLocalRot);
        }
        //Debug.Log("执行了一次复位");
    }
